...
class StripePlanSync extends Command
{
     ...
     * @var string
     */
    protected $signature = 'crons:stripe-plan-sync';
     ...
     * @var string
     */
    protected $description = 'This cron will be run every minute and add plan into stripe system.';
    ...
    public function handle()
    {
        ...
        foreach ($plans as $plan) {
            ...
            if ($plan->plan_stripe_plan && isset($plan->plan_stripe_plan->plan_response_id) && $plan->plan_stripe_plan->deactivated == 1) {

                $planResult = StripeController::createPlan($plan, $result['product_id']);
            } else if ($plan->plan_stripe_plan && isset($plan->plan_stripe_plan->plan_response_id)) {
            } else {
                $planResult = StripeController::createPlan($plan, $result['product_id']);
            }
            if (!empty($planResult)) {
