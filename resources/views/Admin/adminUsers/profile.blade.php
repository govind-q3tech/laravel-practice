<x-layouts.master>
  @section('title', !empty($adminUser) ? 'Manage Profile' : 'Manage Profile')
...
<x-slot name="breadcrumb">
    <div class="row mb-2">
        ...
        <div class="col-sm-6">
          {{ Breadcrumbs::render('common',['append' => [['label' => 'Manage Profile' ]]]) }}
        </div>
      </div>
</x-slot>
<x-slot name="content">
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group row required">
                ...
                <div class="col-sm-6">
                  {{ Form::text('email', old('email') ? old('email') : ((isset($adminUser) && !empty($adminUser->email)) ? $adminUser->email : ''), ['class' => 'form-control' . ($errors->has('email') ? ' is-invalid' : ''), 'placeholder' => 'Email ID','disabled' => true]) }}
                  @error('email')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
