  ...
  + function ($) {
    ...
    ImagesLoader.prototype.ApplyFileValidationRules = function (readerEvt) {
      ...
      if (self.CheckFileType(readerEvt.type) == false) {
        ...
        // " + options.filesType.toString().replaceAll(',', ', ') + "
        self.ShowProgress(self.barStyle.danger, options.errorformat + ": jpeg, png, gif (" + readerEvt.name + ")");
        return false;
      }

      // To check file size according to upload conditions
      if (self.CheckFileSize(readerEvt.size) == false) {
        var sizess = options.maxSize / 1000;
        self.ShowProgress(self.barStyle.danger, options.errorsize + " " + sizess / 1024 + " MB (" + readerEvt.name + ")");
        return false;
      }
